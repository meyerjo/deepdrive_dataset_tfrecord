...
class DeepdriveDatasetWriter(object):
    ...
    def write_tfrecord(self, fold_type=None, version=None, max_elements_per_file=1000, write_masks=False,
        ...
        old_files = DeepdriveDatasetDownload.filter_files(os.path.join(full_labels_path, 'train'))
        print('Number of old/new files: {0} / {1}'.format(len(old_files), len(obj_annotation_dict.keys())))
        for file_counter, f in enumerate(image_files):
            ...
            if not new_format:
                logger.error('Cannot compare if new files not present')
                exit(-1)


            picture_id_annotations_old_format = DeepdriveDatasetWriter.get_annotation(
                picture_id, full_labels_path=os.path.join(full_labels_path, 'train'))

            picture_id_annotations_new_format = obj_annotation_dict.get(picture_id, None)

            # im = Image.open(os.path.join(full_images_path, '{0}.jpg'.format(picture_id)))
            if picture_id_annotations_new_format is None and picture_id_annotations_old_format is not None:
                print('Image Id missing in new set: {0}'.format(picture_id))
                # im.show()
                continue

            if picture_id_annotations_new_format is not None and picture_id_annotations_old_format is None:
                print('Image Id missing in old set: {0}'.format(picture_id))
                continue

            old_boxes = [box for box in picture_id_annotations_old_format['frames'][0]['objects'] if 'box2d' in box]
            new_boxes = [box for box in picture_id_annotations_new_format['labels'] if 'box2d' in box]

            if len(old_boxes) != len(new_boxes):
                print('Picture-Id: {0}\t{1} vs. {2}'.format(picture_id, len(old_boxes), len(new_boxes)))

            for old_box in old_boxes:
                np_old_boxes = np.asarray([old_box['box2d']['y1'], old_box['box2d']['x1'], old_box['box2d']['y2'], old_box['box2d']['x2']])
                for new_box in new_boxes:
                    np_new_boxes = np.asarray([new_box['box2d']['y1'], new_box['box2d']['x1'], new_box['box2d']['y2'], new_box['box2d']['x2']])
                    if np.all(np_old_boxes == np_new_boxes):
                        break
                else:
                    # if im is not None:
                    #     draw = ImageDraw(im)
                    #     draw.rectangle(xy=((np_old_boxes[1], np_old_boxes[0]), (np_old_boxes[3], np_old_boxes[2])),
                    #                    outline='red')
                    #     del draw
                    #     im.show()
                    print('\t{1}\tNo representation found for: {0}'.format(np_old_boxes, picture_id))
